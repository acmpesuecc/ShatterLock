...
int get_subdirectories(const char *parent, char subdirs_out[][256], int max_subdirs) { //-shashi
    ...
    if (!dir) {
        perror("CRITICAL ERROR: 'storage' parent directory not found or inaccessible"); // Added error message
        return 1;
    }
...
int getuniquetouserseed(int *keystream, int len_of_keystream){
    if (keystream == NULL || len_of_keystream <= 0) {
        printf("KEY GENERATION WARNING: Keystream is empty or NULL. Returning zero seed.\n"); // Added warning message
        return 0;
    }
    long long int sum=0;
...
int makekey(char *usn, char *pwd, int *keystream_out){ //here, key depends on the element in front of it.
    ...
    if (len_of_key == 0) {
        printf("KEY GENERATION ERROR: Username or Password length is zero.\n"); // Added error message
    }
...
void vigenerre_encrypt(char *plaintext, char *ciphertext_out, int *keystream, int len){
    for(int i=0;i<strlen(plaintext);i++){
        if (plaintext[i] < 'a' || plaintext[i] > 'z') {
            printf("VIGENÈRE ENCRYPTION WARNING: Character '%c' at index %d is not a lowercase letter (a-z). Result may be unexpected.\n", plaintext[i], i); // Added warning message
        }
...
void vigenerre_decrypt(char *ciphertext, char *plaintext_out, int *keystream, int len){
    for(int i=0;i<strlen(ciphertext);i++){
        if (ciphertext[i] < 'a' || ciphertext[i] > 'z') {
            printf("VIGENÈRE DECRYPTION ERROR: Character '%c' at index %d is outside the 'a'-'z' range. Key or file corruption suspected.\n", ciphertext[i], i); // Added error message
        }
